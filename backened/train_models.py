...
BASE = PROJECT_ROOT / "milestone-2" / "infosys"
...
DAILY_MODEL_ALIASES = {
    "CHAINLINK_INR": "CHAINLINK_COIN_INR",
    "BINANCE_INR": "BINANCE_COIN_INR",
}


def selected_model_names(coins: str | None, freq: str) -> set[str] | None:
    if not coins or coins.strip().lower() == "all":
        return None

    selected = set()
    for raw_item in coins.split(","):
        item = raw_item.strip().upper()
        if not item:
            continue
        model_name = COIN_ALIASES.get(item, item)
        if freq == "daily":
            model_name = DAILY_MODEL_ALIASES.get(model_name, model_name)
...
def build_model(window: int, horizon: int):
...
def train_for_frequency(
    ...
    mlflow.set_tracking_uri((PROJECT_ROOT / "mlflow.db").as_uri().replace("file:///", "sqlite:///"))
    ...
    selected = selected_model_names(coins, freq)
    for csv_name, model_name in COIN_FILES[freq]:
        ...
        with mlflow.start_run(run_name=f"{freq}_{model_name}"):
            mlflow.log_params({
                "frequency": freq,
                "coin": model_name,
                "window": window,
            ...
            mlflow.keras.log_model(
                model,
                "model",
                registered_model_name=f"crypto_{freq}_{model_name.lower()}",
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--coins",
        default="all",
        help="Comma-separated symbols/model names to train, e.g. BTC,ETH,SOL. Use all for every retained coin.",
    )
